...
* **Cost-Aware Scope Scaling:** To mitigate run-away API costs and timeout errors, the generation phase processed the full 100-row baseline, but the final evaluation matrix was strategically scaled down.
* **Building a "Smart Resume" Architecture:** When API latency caused a hard billing timeout mid-execution, a recovery script was built to extract partial data from active runtime memory and dynamically resume. However, a serialization bug occurred where the evaluation framework passed live memory objects (e.g., <anthropic.Anthropic object>) into the dataframe rather than clean strings. This broke the string-matching resume logic.
* **The Final Matrix:** Despite the resume logic failure, the script successfully executed 507 valid evaluations before the final timeout. Rather than spending additional API credits to force a perfect 750-row matrix, the 507-row matrix was deemed statistically significant and retained for the final bias analysis, requiring only a lightweight data-cleaning function to map the memory objects back to string names.
